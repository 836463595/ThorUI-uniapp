...
MyRequest.prototype.request = function(config = {}) {
    ...
    // TODO: 这个链式回调需考虑下 区分request response 
    /**
     * 链式合并
     * 合并顺序格式
     * 没有就会跳过
    ...
    chain.forEach((item) => {
        ...
        promise = promise[type]((obj) => {
            ...
            // TODO: LXJ 这里需要 处理一下 返回的 Data里面的确有可能非config的情况
            // return config(Object类型) 或者 数组 或 return Promise.reject('xx') 才会继续发送请求或回传数据
            if (Tools.isType('Object', ret) || Tools.isType('Array', ret) || Tools.isType('Promise', ret)) {
                return ret;
            }
        });
    });
